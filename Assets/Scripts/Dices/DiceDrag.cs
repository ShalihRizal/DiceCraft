...
public class DiceDrag : MonoBehaviour
{
    ...
    void HighlightDice(DiceDrag dice)
    {
        ...
        dice.transform.DOScale(dice.originalScale * 0.85f, 0.1f).SetEase(Ease.OutQuad);
    }
        ...
        if (wasActuallyDragged)
        {
                ...
                if (hit.CompareTag("Dice") && hit.gameObject != this.gameObject)
                {
                    ...
                    if (otherDice != null && otherDice != this)
                    {
                            ...
                            otherLevel < otherMax)
                        {
                            ...
                            {
                                Vector3 effectPos = otherDice.parentCell != null ? otherDice.parentCell.position : otherDice.transform.position;
                                Instantiate(mergeEffectPrefab, effectPos, Quaternion.identity);
                            }
                        ...
                        else
                        {
                            ...
                            {
                                Vector3 effectPos = this.parentCell != null ? this.parentCell.position : transform.position;
                                Instantiate(dropEffectPrefab, effectPos, Quaternion.identity);
                            }
            ...
            if (spawner != null)
            {
                ...
                if (nearestCell != null && !spawner.IsCellOccupied(nearestCell))
                {
                    ...
                    if (dropEffectPrefab != null)
                    {
                        Vector3 effectPos = nearestCell != null ? nearestCell.position : transform.position;
                        Instantiate(dropEffectPrefab, effectPos, Quaternion.identity);
                    }
                    return;
                }
